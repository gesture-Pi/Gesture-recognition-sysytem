...
void* ListenPort(void* avg)
{
    ...
    if (!myserial.setup(115200, 0, 8, 1, 'N')) //串口初始化
    {
    ...
    while (true)
    {
        if (nread = myserial.readBuffer(buff, 8, Train_num) == 1)  //开始串口监听，等待数据接收  
        {
            continue;
        }
        else
...
int main()
{
    ...
    {
        cout << "Plese train the gesture as follow" << endl;  //不使用现有数据集，重新录入
        //训练函数
        Train_to_Dataset(Mode);
    }
	...
	while (true)
	{
        ...
        if (Trainning_Flag == 1)
        {
            ...
            cout << "······" << endl;
            Filerecv_num = Train_num;  //使串口接收的文件编号为测试集编号
